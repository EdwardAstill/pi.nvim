...
H.test("opening the project chat expands its window to full editor width", function()
  local cc, calls, state = fake_frontend()
  with_modules(frontend_stubs(cc, state), function()
    local bridge = require("pi.codecompanion")
    local root = H.tmpdir()
    ...
    vim.api.nvim_win_set_width(win, 10)
    local columns = vim.o.columns

    local toggled = bridge.toggle(root)
    H.truthy(toggled and toggled.bufnr == chat.bufnr)
